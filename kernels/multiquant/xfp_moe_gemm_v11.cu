...
// XFP MoE GEMM v11 — thin wrapper around xfp_gemm_core template.
//
// Same algorithm as MoE v10 (SHFL codebook lookup + unified K-loop).
// The inner loop lives in xfp_gemm_core.cuh and is shared with the
// Linear variant. Only the MoEPolicy prologue (expert routing) and
// epilogue (topk_weights multiply) are MoE-specific.
...
void xfp_moe_gemm(
    ...
    torch::Tensor topk_weights,      // [M * top_k] float32 (may be empty)
    int64_t bits,
    int64_t K,
    int64_t N,
    int64_t top_k,
    ...
    int64_t num_valid_tokens)
{
    ...
    if (bits == 2)      launch_moe<2>(A, B_packed, codebook, C, sorted_token_ids, expert_ids, topk_weights,
                                      ...
                                      static_cast<int>(top_k), static_cast<int>(flat_per_expert),
                                      static_cast<int>(num_valid_tokens));
    else TORCH_CHECK(false, "xfp_moe_gemm: unsupported bits=", bits);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("xfp_moe_gemm", &multiquant::xfp_moe_gemm,
          "XFP MoE v11: wrapper around shared core template");
}
