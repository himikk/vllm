...
// XFP Linear GEMM v12 — A-row SMEM cache (static, rigid K_SMEM_MAX).
//
// Identical algorithm to v11, but the block-uniform A-row is loaded once
// cooperatively into __shared__ s_A[K_SMEM_MAX]. All 8 warps of the block
// then read A from SMEM instead of each warp re-fetching from global.
//
// Correctness invariant: output bitwise identical to v11.
    ...
    int64_t K)
{
    TORCH_CHECK(A.is_cuda() && B_packed.is_cuda() &&
                ...
                "xfp_gemm v12: all tensors must be CUDA");
    ...
    TORCH_CHECK(K % 2 == 0,
                "xfp_gemm v12: K must be even (bf162 vector load), got K=", K);
    TORCH_CHECK(K <= LinearPolicy::K_SMEM_MAX,
                "xfp_gemm v12: K=", K, " exceeds K_SMEM_MAX=",
                LinearPolicy::K_SMEM_MAX, " — use XFP_KERNEL=v11 for this shape");
    ...
    else TORCH_CHECK(false, "xfp_gemm v12: unsupported bits=", bits);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("xfp_gemm", &multiquant::xfp_gemm,
          "XFP v12: Linear with static SMEM A-row cache");
}
