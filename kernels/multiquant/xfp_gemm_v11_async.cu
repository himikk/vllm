...
// XFP Linear GEMM v11 ASYNC — cp.async double-buffered variant.
//
// Same template as xfp_gemm_v11.cu, but compiled with XFP_CORE_USE_CPASYNC
// which activates the cp.async prefetch path in xfp_gemm_core.cuh.
//
// Correctness invariant: output must be bitwise identical to v11 sync. The
// only difference is how B_packed moves from global to registers.
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("xfp_gemm", &multiquant::xfp_gemm,
          "XFP v11 async: Linear wrapper with cp.async double-buffer");
}
