...
def _tq_quantize_store_kernel(
    ...
    # === 2. Rotate: y = Pi @ x_hat ===
    # Pi is [D, D], we compute y[i] = sum_j Pi[i,j] * x_hat[j]
    ...
    for i in tl.static_range(head_size):
        ...
        # Scatter into y - use masking since we can't index dynamically
    ...
    # For each y[j], find nearest centroid
    # y: [D], centroids: [n_centroids]
    # We need idx[j] = argmin_k |y[j] - centroids[k]|
    # Process element-by-element (D iterations, each finding min over n_centroids)
    idx = tl.zeros([head_size], dtype=tl.int32)
    y_hat = tl.zeros([head_size], dtype=tl.float32)

    for j in tl.static_range(head_size):
        y_j = tl.sum(tl.where(d_offs == j, y, 0.0))  # extract y[j]
        # Compute distances to all centroids
        dists = tl.abs(y_j - centroids)  # [n_centroids]
    ...
    for i in tl.static_range(head_size):
        # Pi^T[i, :] = Pi[:, i], i.e., column i of Pi
    ...
    if mse_bits == 2:
        byte_offs = tl.arange(0, mse_bytes)
    ...
    # Convert to float16 and store raw bytes
    norm_f16 = vec_norm.to(tl.float16)
    gamma_f16 = gamma.to(tl.float16)
    # Store as uint16 reinterpreted
    ...
    # Store as FP16 raw bytes into value cache
    val_cache_base = (
        block_idx * block_size * num_heads * packed_size
        + block_off * num_heads * packed_size
        + head_idx * packed_size
    )
    # Store first packed_size bytes of the FP16 value
    # Each FP16 element = 2 bytes, so we can fit packed_size//2 elements
    n_val_elems = packed_size // 2
    val_offs = tl.arange(0, head_size)
    # Only store what fits
    v_store = tl.where(val_offs < n_val_elems, v_vec, 0.0)
    # We store element-by-element as the cache is uint8
    for e in tl.static_range(n_val_elems):
        if e < head_size:
            v_e = tl.sum(tl.where(val_offs == e, v_vec, 0.0)).to(tl.float16)
            # Bitcast float16 to uint16, then split into 2 bytes
